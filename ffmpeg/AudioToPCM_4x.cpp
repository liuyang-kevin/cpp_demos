...
g++ -std=c++11 -I /usr/local/include AudioToPCM_4x.cpp  \
-lavdevice -lavfilter \
-lavformat -lavcodec -lswresample -lswscale -lavutil \
-lsdl2 -lsdl2_image \
-o run.out  && \
./run.out ./media/test.mp4
...
typedef struct PacketQueue {
  ...
  SDL_mutex *
      mutex;  //互斥锁; 因为SDL
              //是在一个独立的线程中来进行音频处理的。如果我们没有正确的锁定这个队列，我们有可能把数据搞乱。
  SDL_cond *cond;  //条件变量
} PacketQueue;
...
int audio_decode_frame(AVCodecContext *aCodecCtx, uint8_t *audio_buf,
  ...
  for (;;) {
    ...
    av_packet_unref(&pkt);
    if (quit) return -1;
    if (packet_queue_get(&audioq, &pkt, 1) <
        0) {  //从这里开始，取得main线程放入队列的包
...
void audio_callback(void *userdata, Uint8 *stream, int len) {
  ...
  while (len > 0) {
    if (audio_buf_index >=
        audio_buf_size) {  // already send all our data, get more
...
int video_thread_proc(void *data) {
  ...
  // 实际视频显示是以 PTS 为准的，但是 PTS
  // 大多没有规律（只要是在解压之后就好），难以处理 所幸在相当一部分视频中 DTS
  // 与 PTS 相差较小，所以这里就用 DTS 了
  ...
  while (!video_quit) {
    while (!frameq.empty()) {
      ...
      if (pFrame == NULL) {
        ...
        // 这里采用了“连续读取”的方法，也即如果下一帧的 DTS
        // 小于当前基准（目前采用音频流），则循环直至找到第一个 DTS 比基准大的
        // 然后播放小于基准而且离基准最近的帧，并缓存下一帧
        // 由于处理时间较长，而且使用了互斥体（看，音频部分也用到了），所以可能有极少数的音频帧有异常（噪声啊之类的）
        // 当然也可以将这一段注释掉，采用 pFrame = frameq.count();
        // frameq.pop();，同时解除 if (packet_queue_get())
        // 的注释（和下面的一对大括号） 但是无跳过的方法会导致视频严重滞后
        ...
        sws_scale(params->sws_context, (const uint8_t *const *)pFrame->data,
                  pFrame->linesize, 0, params->vid_codec_context->height,
                  g_pFrameYUV->data, g_pFrameYUV->linesize);

        SDL_UpdateYUVTexture(params->texture, NULL, g_pFrameYUV->data[0],
                             g_pFrameYUV->linesize[0], g_pFrameYUV->data[1],
                             g_pFrameYUV->linesize[1], g_pFrameYUV->data[2],
                             g_pFrameYUV->linesize[2]);
...
int main(int argc, char *argv[]) {
  ...
  // pCodecCtx->refcounted_frames = 1; // 4.0 codec过时
  // 老版本先预加载1帧？然后取codec
  ...
  pCodecCtx =
      avcodec_alloc_context3(pCodec);  //需要使用avcodec_free_context释放
  avcodec_parameters_to_context(pCodecCtx, pCodecParam);

  paCodecCtx =
      avcodec_alloc_context3(paCodec);  //需要使用avcodec_free_context释放
  avcodec_parameters_to_context(paCodecCtx, paCodecParam);

  if (pCodec == NULL ||
      paCodecParam == NULL) {  // 从视频角度出发，不合适就不解码
  ...
  printf("比特率 %3lld\n", pFormatCtx->bit_rate);
  printf("解码器名称 %s\n", paCodec->long_name);
  printf("time_base  %d/%d \n", paCodecCtx->time_base.num,
         paCodecCtx->time_base.den);
  ...
  int numBytes;
  numBytes =
      av_image_get_buffer_size(AV_PIX_FMT_YUV420P, pCodecParam->width,
                               pCodecParam->height, 1 /*(按照多少字节对齐)*/);
  //   numBytes = avpicture_get_size(AV_PIX_FMT_YUV420P, pCodecParam->width,
  //                                 pCodecParam->height);
  ...
  av_image_fill_arrays(g_pFrameYUV->data, g_pFrameYUV->linesize, out_buffer,
                       AV_PIX_FMT_YUV420P, pCodecCtx->width, pCodecCtx->height,
                       1);

  //   avpicture_fill((AVPicture *)g_pFrameYUV, out_buffer, AV_PIX_FMT_YUV420P,
  ...
  window = SDL_CreateWindow("MPlayer", SDL_WINDOWPOS_CENTERED,
                            SDL_WINDOWPOS_CENTERED, pCodecParam->width,
                            pCodecParam->height, SDL_WINDOW_SHOWN);
  if (!window) {
  ...
  texture =
      SDL_CreateTexture(ren, SDL_PIXELFORMAT_YV12, SDL_TEXTUREACCESS_STREAMING,
                        pCodecParam->width, pCodecParam->height);
  ...
  img_convert_ctx = sws_getContext(
      pCodecParam->width, pCodecParam->height,
      (AVPixelFormat)(pCodecParam->format), pCodecParam->width,
      pCodecParam->height, AV_PIX_FMT_YUV420P, SWS_BICUBIC, NULL, NULL, NULL);
  ...
  g_pVideoThread =
      SDL_CreateThread(video_thread_proc, "video_thread",
                       &vtp);  // 原demo使用了queue 这里注释了代码 执行了空方法
  ...
  while ((!quit) && (av_read_frame(pFormatCtx, &packet) >= 0  // 抽取一包数据
                     || (!frameq.empty()))) {
