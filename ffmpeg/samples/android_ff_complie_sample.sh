...
make clean
#你自己的NDK路径.
export NDK=/Users/fd/Android/ndk-bundle
...
# Program options:命令行工具，用动态库的可以不用编译这一组
...
# Component options: 大组件的编译选项
# Libavdevice库可以读取/输出电脑（或者其他设备）的多媒体数据
# 可以操作我们的摄像头（但是在Android中是不支持的），视频输出等等
export COMMON_FF_CFG_FLAGS="$COMMON_FF_CFG_FLAGS --disable-avdevice"
# 包含了音视频编解码的东西，默认是开启的，肯定不能关闭掉
...
# 离散余弦变换（DCT）。它常被认为是对语音和图像信号进行变换的最佳方法。
# export COMMON_FF_CFG_FLAGS="$COMMON_FF_CFG_FLAGS --disable-dct"
# DTW为(Dynamic Time Warping,动态时间归准). 例如：孤立词语音识别，计算机视觉中的行为识别，信息检索等中
# export COMMON_FF_CFG_FLAGS="$COMMON_FF_CFG_FLAGS --disable-dwt"
# 线谱对(LSP)，例如：分析在G.729语音编码算法中的应用
...
# 改进离散余弦变换，应用于语音、宽带音频和图像信号的变换编码中
...
# AAN算法 Arai-Agui-Nakajima (AAN) 
#   --disable-faan           disable floating point AAN (I)DCT code
# 视频编码中要应用各种抗误码(Error Resilience)技术。抗误码技术又称“差错控制方法”。
...
export COMMON_FF_CFG_FLAGS="$COMMON_FF_CFG_FLAGS --disable-bzlib"
    ...
    # 编译带h264_mediacodec版本的库需要开启以下选项， 3.1版本加入了android mediacodec硬解支持
    # --enable-jni \                          # java支持， 
    # --enable-mediacodec \                   # 硬解？
    # --enable-decoder=h264_mediacodec \      # 硬解模式
    # --enable-hwaccel=h264_mediacodec \      # 硬件加速
        # --enable-jni \
    # --enable-mediacodec \
